...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:dw="http://www.ditaworks.com/xslt"
    extension-element-prefixes="exsl"
    exclude-result-prefixes="opentopic exsl opentopic-index dita2xslfo dw"
    version="2.0">
    ...
    <xsl:variable name="id.index" select="'_OPENTOPIC_TOC_PROCESSING_INDEX'"/>
    ...
    <xsl:template match="*" mode="commonTopicProcessing">
        ...
        <!-- <xsl:if test="*[contains(@class, 'task/task')]">
            <xsl:variable name="level" select="count(ancestor::*[contains(@class,' topic/topic ')])"/>
            <xsl:if test="$level = 2">
                <xsl:attribute name="padding-top">-30pt</xsl:attribute>
            </xsl:if>
        </xsl:if> -->
        <!-- <xsl:if test="*[contains(@class, 'task/task')]">
            <fo:inline>
                <xsl:value-of select="following-sibling::*[1][contains(@class, 'task/task')]"/>
            </fo:inline>
        </xsl:if> -->
        <!-- <xsl:if test="*[contains(@class, 'task/task')]">
            <xsl:variable name="level" select="count(ancestor::*[contains(@class,' topic/topic ')])"/>
	...
	<!-- <xsl:template match="text()[parent::*[not(contains(@class,'pr-d/codeph')) and not(contains(@class,'pr-d/codeblock'))]]"> -->
    <xsl:template match="text()">
		<xsl:analyze-string select="." regex="([^ =\n]*\-[^ =\n]*)+">
			<xsl:matching-substring>
				<fo:inline keep-together.within-line="always"><xsl:value-of select="."/></fo:inline>
			</xsl:matching-substring>
...
<!-- jko: 2006-04-21 new -->
                                    ...
                                    <xsl:variable name="type">
                                        <xsl:choose>
                                            <xsl:when test="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@fsTocType = 'suppress'">
                                                <xsl:value-of select="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@type"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:value-of select="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@fsTocType"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:variable>

                                    <xsl:call-template name="insertStaticContents">
                                        <xsl:with-param name="pagetype" select="$type"/>
                                    </xsl:call-template>
<!-- end -->
...
<!-- 2CHECK -->
                    ...
                    <fo:list-block provisional-distance-between-starts="40pt" start-indent="-40pt">
                        <fo:list-item>
    ...
    <xsl:template name="processTopicAppendix">
        <fo:page-sequence master-reference="body-sequence" xsl:use-attribute-sets="__force__page__count">
            ...
            <xsl:call-template name="insertStaticContents">
                <xsl:with-param name="pagetype" select="$fsTocType"/>
            </xsl:call-template>
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:if test="not(ancestor::*[contains(@class, ' topic/topic ')])">
                        <fo:marker marker-class-name="current-topic-number">
                            <xsl:if test="$fsTocType != 'glossary' ">
                                <xsl:value-of select="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@number"/>
                                <!--<xsl:value-of select="$appendixNumber"></xsl:value-of>-->
                            </xsl:if>
                        </fo:marker>
                        <fo:marker marker-class-name="current-header">
                            <xsl:for-each select="child::*[contains(@class,' topic/title ')]">
                    ...
                    <fo:block xsl:use-attribute-sets="topic.title">
                        ...
                        <xsl:attribute name="margin-bottom">10.5pt</xsl:attribute>
                        <!-- added by William on 2009-07-02 for indexterm bug:2815485 start-->
                        <xsl:call-template name="pullPrologIndexTerms"/>
                        <!-- added by William on 2009-07-02 for indexterm bug:2815485 end-->
                        <xsl:variable name="id" select="@id"/>
                        <xsl:choose>
                            <xsl:when test="$fsTocType != 'glossary' ">
                                <xsl:call-template name="insertVariable">
                                    <xsl:with-param name="theVariableID" select="'header.appendix'"/>
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:value-of select="$glossaryTitle"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </fo:block>
...
<!-- New -->
        <xsl:variable name="id" select="parent::node()/@id"/>
        <xsl:variable name="number" select="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@number"/>
        <xsl:variable name="guid" select="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@guid"/>
<!-- jko: 2006-04-21 new -->
        <xsl:variable name="type" select="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@type"/>
        <!-- Use own "level" -->
        <xsl:variable name="toclevel" select="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@level"/>
        <xsl:variable name="fsTocType" select="exsl:node-set($fs_topicNumbers)//*[@id = $id]/@fsTocType"/>
<!-- end -->
<!-- End  -->
        ...
        <fo:block>
            ...
            <fo:block>
                ...
                <fo:block id="{parent::node()/@id}" height="0"/>
                <!-- <fo:inline>
                    <xsl:attribute name="id">
                        <xsl:call-template name="generate-toc-id">
                ...
                <xsl:variable name="formattedNumber">
                    <xsl:if test="($type = 'chapter' or $type = 'appendix') and $fsTocType != 'glossary'">
                        <xsl:if test="$type = 'appendix' and $toclevel = 1">
                            <xsl:call-template name="insertVariable">
                                <xsl:with-param name="theVariableID" select="'Appendix'"/>
                            </xsl:call-template>
                        </xsl:if>
                ...
                <xsl:choose>
                    <!-- appendix text is too long for 40 pt -->
                    <xsl:when test="$type = 'appendix' and $toclevel = 1">
                    ...
                    <!-- level 1 without number (glossary, Index) -->
...
<!-- ######################################################################################### -->
<!-- GS 2006-12-04: Commented out because otherwise prodname is not displayed on the title page -->
...
<!-- ######################################################################################### -->
<!-- GS 2006-12-04: Commented out because otherwise brand is not displayed on the title page -->
...
<!-- ######################################################################################### -->
<!-- GS 2007-04-10: Commented out because otherwise series is not displayed on the title page -->
...
<!-- ######################################################################################### -->
<!-- GS 2006-12-04: Commented out because otherwise prognum is not displayed on the title page -->
...
<!-- ######################################################################################### -->
<!-- GS 2006-12-04: Commented out because otherwise component is not displayed on the title page -->
...
<!--             add keycol here once implemented-->
  ...
  <xsl:template match="*[contains(@class,' topic/section ')][@spectitle != '' 
    ...
    <xsl:template match="*[contains(@class,' topic/section ')]">
        <xsl:if test="dw:isElementEmpty(.) = false()">
    ...
    <xsl:function name="dw:isElementEmpty">
        <xsl:param name="node"/>
        <xsl:choose>
            <xsl:when test="count($node/child::*) > 0">
                <xsl:value-of select="false()" />
            </xsl:when>
            <xsl:when
                test="(count($node/child::node()) = 1) and (string-length(normalize-space($node/child::node()[1])) = 0)">
...
-->
    ...
    <xsl:template match="*[contains(@class, ' topic/p ')]">
        <fo:block xsl:use-attribute-sets="p">
            <xsl:call-template name="commonattributes"/>            
            <xsl:if test="preceding-sibling::*[1][contains(@class, ' topic/table ')]">
                <xsl:attribute name="space-before">11pt</xsl:attribute>
            </xsl:if>
            <!-- <xsl:if test="../contains(@class, ' topic/li ')">
        ...
        <xsl:choose>
            <xsl:when test="not($noteImagePath = '')">
                <fo:table xsl:use-attribute-sets="note__table">
                    <fo:table-column xsl:use-attribute-sets="note__image__column" column-width="36pt"/>
                    <fo:table-column xsl:use-attribute-sets="note__text__column" column-width="399.35pt"/>
                    <fo:table-body>
                        <fo:table-row>
                                <fo:table-cell xsl:use-attribute-sets="note__text__entry" column-number="1">
                                    <fo:block xsl:use-attribute-sets="note">
                                    <!--     <fo:external-graphic src="url({concat($artworkPrefix, $noteImagePath)})" xsl:use-attribute-sets="image"/> -->
                                    <fo:inline xsl:use-attribute-sets="note__label">
<!-- GS 2006-11-06: Replaced hard-coded text by function that gets the language-dependant text -->
<!--                                            <xsl:text>Note: </xsl:text> -->
                                ...
                                <fo:table-cell xsl:use-attribute-sets="note__text__entry"  column-number="2">
            ...
            <xsl:if test="not($width) and not($height)">
	            <xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:attribute name="width">270pt</xsl:attribute>
...
<!-- ######################################################################################### -->
    <!-- Templates to process definition lists and their entries -->
    ...
    <xsl:template match="*[contains(@class, ' topic/dlentry ')]">
        <fo:block xsl:use-attribute-sets="dlentry" id="{@id}">
                <xsl:apply-templates/>
        </fo:block>
    </xsl:template>

    <xsl:template match="*[contains(@class, ' topic/dt ')]">
        <fo:block xsl:use-attribute-sets="dt" id="{@id}">
                <xsl:apply-templates/>
        </fo:block>
    </xsl:template> 
    <xsl:template match="*[contains(@class, ' topic/dd ')]">
        <fo:block xsl:use-attribute-sets="dd" id="{@id}">
                <xsl:apply-templates/>
        </fo:block>
    </xsl:template> 
