...
public class BillingDataRetrievalServiceBeanRolePricesIT extends EJBTestBase {
	...
	public void testGetRoleDefinitionsForPriceModelNonExistingPM()
			throws Exception {
		Map<Long, RoleDefinitionHistory> roleDefinitionsForPriceModel = runTX(new Callable<Map<Long, RoleDefinitionHistory>>() {
			public Map<Long, RoleDefinitionHistory> call() throws Exception {
	...
	public void testGetRoleDefinitionsForPriceModelOneHit() throws Exception {
		initData(1, false);
		Map<Long, RoleDefinitionHistory> roles = runTX(new Callable<Map<Long, RoleDefinitionHistory>>() {
			public Map<Long, RoleDefinitionHistory> call() throws Exception {
				return bdrs.loadRoleDefinitionsForPriceModel(subscription
	...
	public void testGetRoleDefinitionsForPriceModelRoleDefForDifferentProduct()
		...
		Map<Long, RoleDefinitionHistory> roles = runTX(new Callable<Map<Long, RoleDefinitionHistory>>() {
			public Map<Long, RoleDefinitionHistory> call() throws Exception {
				return bdrs.loadRoleDefinitionsForPriceModel(priceModelKey,
						System.currentTimeMillis() + 5000);
			}
		});
		...
		runTX(new Callable<Void>() {
			...
			public Void call() throws Exception {
				...
				long rdKey = technicalProduct.getRoleDefinitions().get(0)
						.getKey();
				Query query = mgr
						.createQuery("UPDATE RoleDefinitionHistory rdh SET rdh.modDate = :date WHERE rdh.objKey = :objKey");
	...
	public void testGetRoleDefinitionsForPriceModelCheckVersionForEditedEntry()
		...
		Assert.assertEquals(1, roles.get(Long.valueOf(keyArray[0]))
				.getObjVersion());
		Assert.assertEquals("roleDefinition1",
				roles.get(Long.valueOf(keyArray[1])).getRoleId());
		Assert.assertEquals(0, roles.get(Long.valueOf(keyArray[1]))
				.getObjVersion());
	}

	@Test
	public void testGetRoleRelatedCostsForPriceModelNoHit() throws Exception {
		Map<Long, RolePricingDetails> result = runTX(new Callable<Map<Long, RolePricingDetails>>() {
			public Map<Long, RolePricingDetails> call() throws Exception {
				return bdrs.loadRoleRelatedCostsForPriceModel(1111L, 1111L);
			}
		});
	...
	public void testGetRoleRelatedCostsForPriceModelOneHit() throws Exception {
		...
		Map<Long, RolePricingDetails> result = runTX(new Callable<Map<Long, RolePricingDetails>>() {
			public Map<Long, RolePricingDetails> call() throws Exception {
				return bdrs.loadRoleRelatedCostsForPriceModel(subscription
						.getPriceModel().getKey(),
						System.currentTimeMillis() + 5000);
			}
		});
	...
	public void testGetRoleRelatedCostsForPriceModelTwoHits() throws Exception {
		...
		Assert.assertEquals(new BigDecimal(12), result.get(keys.get(0))
				.getPricePerUser());
		Assert.assertEquals(new BigDecimal(24), result.get(keys.get(1))
				.getPricePerUser());
	}
	...
	public void testGetRoleRelatedCostsForPriceModelOneHitMultipleSubscriptions()
		...
		Map<Long, RolePricingDetails> result = runTX(new Callable<Map<Long, RolePricingDetails>>() {
			public Map<Long, RolePricingDetails> call() throws Exception {
				return bdrs.loadRoleRelatedCostsForPriceModel(priceModelKey,
						System.currentTimeMillis() + 5000);
			}
		});
		Assert.assertNotNull(result);
		...
		Assert.assertEquals(new BigDecimal(12),
				result.get(result.keySet().iterator().next()).getPricePerUser());
	}
		...
		runTX(new Callable<Void>() {
			...
			public Void call() throws Exception {
				...
				PricedProductRole ppr = subscription.getPriceModel()
						.getRoleSpecificUserPrices().get(0);
				Query query = mgr
						.createQuery("UPDATE PricedProductRoleHistory pprh SET pprh.modDate = :modDate WHERE pprh.objKey = :objKey");
	...
	public void testGetRoleRelatedCostsForParametersForOneParamMultiplePricedRoles()
		...
		RolePricingData paramCosts = runTX(new Callable<RolePricingData>() {
			public RolePricingData call() throws Exception {
				return bdrs.loadRoleRelatedCostsForParameters(subscription
						.getPriceModel().getKey(),
						System.currentTimeMillis() + 5000);
			}
		});
		...
		runTX(new Callable<Void>() {
			public Void call() throws Exception {
				...
				PricedProductRole ppr = subscription.getPriceModel()
						...
						.getRoleSpecificUserPrices().get(0);
				Query query = mgr
						.createQuery("UPDATE PricedProductRoleHistory pprh SET pprh.modDate = :modDate WHERE pprh.objKey = :ppKey");
				Calendar cal = Calendar.getInstance();
				cal.add(Calendar.MONTH, 1);
				query.setParameter("modDate", cal.getTime());
	...
	public void testGetRoleRelatedCostsForParametersForOneParamPPRNewVersion()
		...
		Assert.assertEquals(1, pprh.getPricedProductRoleHistory()
				.getObjVersion());
	}
	...
	public void testGetRoleRelatedCostsForOptionsNoHit() throws Exception {
		ParameterOptionRolePricingData optionCosts = runTX(new Callable<ParameterOptionRolePricingData>() {
			public ParameterOptionRolePricingData call() throws Exception {
				return bdrs.loadRoleRelatedCostsForOptions(1111L, 1111L);
			}
		});
	...
	public void testGetRoleRelatedCostsForOptionsOneOption() throws Exception {
		...
		ParameterOptionRolePricingData optionCosts = runTX(new Callable<ParameterOptionRolePricingData>() {
			public ParameterOptionRolePricingData call() throws Exception {
				return bdrs.loadRoleRelatedCostsForOptions(subscription
						.getPriceModel().getKey(),
						System.currentTimeMillis() + 5000);
			}
		});
	...
	public void testGetRoleRelatedCostsForOptionsOneOptionRoleDefOutOfPeriod()
		...
		runTX(new Callable<Void>() {
			public Void call() throws Exception {
				...
				Query query = mgr
						.createQuery("UPDATE PricedProductRoleHistory poh SET poh.modDate = :modDate WHERE poh.objKey = :objKey");
				Calendar cal = Calendar.getInstance();
				cal.add(Calendar.MONTH, 1);
				query.setParameter("modDate", cal.getTime());
				query.setParameter(
						"objKey",
						Long.valueOf(pricedOption.getRoleSpecificUserPrices()
								.get(0).getKey()));
				query.executeUpdate();
				return null;
			}
		...
		ParameterOptionRolePricingData optionCosts = runTX(new Callable<ParameterOptionRolePricingData>() {
			public ParameterOptionRolePricingData call() throws Exception {
				return bdrs.loadRoleRelatedCostsForOptions(priceModel.getKey(),
						System.currentTimeMillis() + 5000);
			}
		});
		Assert.assertNotNull(optionCosts);
	...
	public void testGetRoleRelatedCostsForOptionsOneOptionRoleUpdated()
		...
		Map<Long, RolePricingDetails> optionRolePrices = rolePrices
				.getRolePricesForContainerKey(optionKey);
		Assert.assertEquals(2, optionRolePrices.keySet().size());
		List<Long> roleKeys = new ArrayList<Long>(optionRolePrices.keySet());
	...
	public void testGetRoleRelatedCostsForOptionsMultipleParamsAndOptions()
		...
		List<Long> paramKeys = new ArrayList<Long>(
	...
	private void initData(final int roleCount,
		...
		runTX(new Callable<Void>() {
			...
			public Void call() throws Exception {
				...
				Parameter booleanParam = Products.createParameter(
						booleanParamDef, product, mgr);
				...
				for (int i = 0; i < roleCount; i++) {
					...
					if (createSecondPricedParam) {
						...
						pprForBooleanParam.setPricePerUser(new BigDecimal(
								19 * (i + 1)));
						...
						pprForOption2.setPricePerUser(new BigDecimal(
								27 * (i + 1)));
