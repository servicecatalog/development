...
public class DataAccessService {
			...
			try (PreparedStatement stmt = con.prepareStatement(query2);) {
				for (Datacenter dc : vcenter.datacenter) {
					for (Cluster cluster : dc.cluster) {
						logger.debug("dc: " + dc.name + " vcenter: " + vcenter.tkey + " cluster: " + cluster.name
	...
	private void retrieveDatacenter(VCenter vcenter) throws Exception {
		logger.debug("vcenter: " + vcenter.name + " vcenter_tkey: " + vcenter.tkey);
		...
		try (Connection con = getDatasource().getConnection(); PreparedStatement stmt = con.prepareStatement(query);) {
			...
			logger.error("Failed to retrieve datacenter list for vCenter " + vcenter.name, e);
			throw e;
		}
	...
	private void retrieveCluster(VCenter vc, Datacenter dc) throws Exception {
		...
		try (Connection con = getDatasource().getConnection(); PreparedStatement stmt = con.prepareStatement(query);) {
			...
			logger.error("Failed to retrieve cluster list for vCenter server " + vc.name + " and datacenter " + dc.name,
					e);
			throw e;
		}
	...
	public VMwareCredentials getCredentials(String vcenter) throws Exception {
		...
		try (Connection con = getDatasource().getConnection(); PreparedStatement stmt = con.prepareStatement(query);) {
			...
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				credentials = new VMwareCredentials(rs.getString("url"), rs.getString("userid"),
	...
	public String getDatacenterId(String vcenter, String datacenter) throws Exception {
		...
		try (Connection con = getDatasource().getConnection(); PreparedStatement stmt = con.prepareStatement(query);) {
			...
			logger.error("Failed to retrieve datacenter id from vcenter: " + vcenter + " datacenter: " + datacenter, e);
			throw e;
		}

		if (datacenterId == null) {
			logger.error("No datacenter id defined for vcenter: " + vcenter + " datacenter: " + datacenter);
		}
	...
	public List<VLAN> getVLANs(Cluster cluster) throws Exception {
		...
		logger.debug("cluster: " + cluster.name + " cluster_tkey: " + cluster.tkey + " datacenter_tkey: "
	...
	@SuppressWarnings("resource")
	public VMwareNetwork getNetworkSettings(String vcenter, String datacenter, String cluster, String vlan) throws Exception {
		VMwareNetwork network = new VMwareNetwork();
		logger.debug("vcenter: " + vcenter + "  datacenter: " + datacenter + "  cluster: " + cluster + "  vlan: " + vlan);
	...
	public String getVLANwithMostIPs(String vcenter, String datacenter, String cluster) {
		...
		try (Connection con = getDatasource().getConnection(); PreparedStatement stmt = con.prepareStatement(query);) {
			...
			logger.error("Failed to retrieve vlans for vcenter: " + vcenter + "  datacenter: " + datacenter
					+ "  cluster: " + cluster, e);
			return null;
		}
		...
		if (vlans.length() == 0) {
			logger.error(
					"No vlans found for vcenter: " + vcenter + "  datacenter: " + datacenter + "  cluster: " + cluster);
			return null;
		}
	 ...
	 */
	@SuppressWarnings("resource")
	public String reserveIPAddress(String vcenter, String datacenter, String cluster, String vlan) throws Exception {
		...
		try (Connection con = getDatasource().getConnection();) {
			...
			if (vlanTKey == -1) {
				logger.error("Failed to reserve IP address for vcenter: " + vcenter + "  datacenter: " + datacenter
	 ...
	 */
	@SuppressWarnings("resource")
	public boolean markIPAddressAsUsed(String vcenter, String datacenter, String cluster, String vlan,
			String ipAddress) {
		logger.debug("vcenter: " + vcenter + "  datacenter: " + datacenter + "  cluster: " + cluster + "  vlan: " + vlan
				+ "  ipAddress: " + ipAddress);
	 ...
	 */
	@SuppressWarnings("resource")
	public boolean isIPAddressInUse(String vcenter, String datacenter, String cluster, String vlan, String ipAddress)
			throws Exception {
		logger.debug("vcenter: " + vcenter + "  datacenter: " + datacenter + "  cluster: " + cluster + "  vlan: " + vlan
				+ "  ipAddress: " + ipAddress);
		...
		try (Connection con = getDatasource().getConnection();) {
			...
			if (!foundEntry) {
				...
				String message = Messages.get(locale, "error_db_validate_ip_not_found",
	 ...
	 */
	@SuppressWarnings("resource")
	public boolean isIPAddressPresent(String vcenter, String datacenter, String cluster, String vlan, String ipAddress)
			throws Exception {
		logger.debug("vcenter: " + vcenter + "  datacenter: " + datacenter + "  cluster: " + cluster + "  vlan: " + vlan
				+ "  ipAddress: " + ipAddress);
	 ...
	 */
	@SuppressWarnings("resource")
	public void addIPAddress(String vcenter, String datacenter, String cluster, String vlan, String ipAddress)
			throws Exception {
		logger.debug("vcenter: " + vcenter + "  datacenter: " + datacenter + "  cluster: " + cluster + "  vlan: " + vlan
				+ "  ipaddress: " + ipAddress);
	 ...
	 */
	public void releaseIPAddress(String vcenter, String datacenter, String cluster, String vlan, String ipaddress)
			throws Exception {
	...
	private int getVLANTKey(Connection con, String vcenter, String datacenter, String cluster, String vlan) throws Exception {
		logger.debug("vcenter: " + vcenter + "  datacenter: " + datacenter + "  cluster: " + cluster + "  vlan: " + vlan);
		int vlan_tkey = -1;
	...
	private int getClusterTKey(Connection con, String vcenter, String datacenter, String cluster) throws Exception {
		logger.debug("vcenter: " + vcenter + "  datacenter: " + datacenter + "  cluster: " + cluster);
		int cluster_tkey = -1;
