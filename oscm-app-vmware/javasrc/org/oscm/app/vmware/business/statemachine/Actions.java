...
public class Actions {
	...
	public String configureVM(String instanceId, ProvisioningSettings settings,
		...
		try {
			...
			String message = Messages.get(ph.getLocale(), "error_configure_vm", new Object[] { instanceId });
	...
	@StateMachineAction
	public String checkVMRunning(String instanceId, ProvisioningSettings settings,
			@SuppressWarnings("unused") InstanceStatus result) {
		...
		try {
			...
			String message = Messages.get(ph.getLocale(), "error_check_vm_running",
	...
	@StateMachineAction
	public String finalizeProvisioning(String instanceId, ProvisioningSettings settings, InstanceStatus result) {
		...
		try {
			...
			String message = Messages.get(ph.getLocale(), "error_finalize_provisioning", new Object[] { instanceId });
			ph.setSetting(VMPropertyHandler.SM_ERROR_MESSAGE, message);
			return EVENT_FAILED;
		} finally {
			if (vmClient != null) {
	...
	@SuppressWarnings("unused")
	@StateMachineAction
	public String throwSuspendException(String instanceId, ProvisioningSettings settings, InstanceStatus result) {
		return EVENT_FAILED;
	}

	@StateMachineAction
	public String inspectTaskResult(String instanceId, ProvisioningSettings settings,
			@SuppressWarnings("unused") InstanceStatus result) {
		...
		try {
			...
			case ERROR:
				...
				if (taskInfo != null && taskInfo.getError() != null) {
					...
					if (taskInfo.getError().getFault() != null) {
						List<LocalizableMessage> errorMessageList2 = taskInfo.getError().getFault().getFaultMessage();
						...
						if (taskInfo.getError().getFault().getFaultCause() != null) {
							String errorMsg = taskInfo.getError().getFault().getFaultCause().getLocalizedMessage();
			...
			String message = Messages.get(ph.getLocale(), "error_check_task_result",
	...
	protected String successfulTask(@SuppressWarnings("unused") TaskInfo taskInfo, VMPropertyHandler ph) {
		ph.setSetting(VMPropertyHandler.GUEST_READY_TIMEOUT_REF, String.valueOf(System.currentTimeMillis()));
		return EVENT_SUCCESS;
	}

	protected TaskInfo getTaskInfo(VMwareClient vmw, VMPropertyHandler paramHandler) throws Exception {
		...
		ManagedObjectReference taskManagerRef = vmw.getConnection().getServiceContent().getTaskManager();
		TaskFilterSpec taskfilter = new TaskFilterSpec();
		ManagedObjectReference taskHistoryCollector = vmw.getConnection().getService()
				.createCollectorForTasks(taskManagerRef, taskfilter);
		vmw.getConnection().getService().resetCollector(taskHistoryCollector);
		vmw.getConnection().getService().readNextTasks(taskHistoryCollector, 100);
		List<TaskInfo> taskList = vmw.getConnection().getService().readPreviousTasks(taskHistoryCollector, 100);
	...
	private void logTaskInfo(TaskInfo info) {
		...
		String completeTime = completeT != null ? completeT.toGregorianCalendar().getTime().toString() : "";
		logger.debug(key + " name: " + name + " target: " + target + " state: " + state + " progress: " + progress
				+ "% description: " + description + " initiated: " + initiatedBy + " queue-time: " + queueTime
				+ " start-time: " + startTime + " complete-time: " + completeTime);
	}
