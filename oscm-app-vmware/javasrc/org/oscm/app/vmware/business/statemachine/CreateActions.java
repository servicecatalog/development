...
public class CreateActions extends Actions {
	...
	@StateMachineAction
	public String createInstanceName(String instanceId, ProvisioningSettings settings,
		...
		if (regex != null) {
			...
			if (!m.matches()) {
				logger.error("Validation error on instance name: [" + instanceName + "/" + regex + "] for instanceId"
						+ instanceId);
	...
	public String executeScript(String instanceId, ProvisioningSettings settings,
		...
		try {
			...
			String message = Messages.get(ph.getLocale(), "error_execute_script", new Object[] { instanceId });
			ph.setSetting(VMPropertyHandler.SM_ERROR_MESSAGE, message);
			return EVENT_FAILED;
		} finally {
			if (vmClient != null) {
	...
	@StateMachineAction
	public String suspendAfterCreation(String instanceId, ProvisioningSettings settings, InstanceStatus result) {
		...
		try {
			...
			String message = Messages.get(ph.getLocale(), "error_pause_after_creation", new Object[] { instanceId });
			ph.setSetting(VMPropertyHandler.SM_ERROR_MESSAGE, message);
			return EVENT_FAILED;
		}
	}

	private void sendEmail(VMPropertyHandler paramHandler, String instanceId, String mailRecipient) throws Exception {
		...
		String subject = Messages.get(paramHandler.getSettings().getLocale(), "mail_pause_after_creation.subject",
		...
		String text = Messages.get(paramHandler.getSettings().getLocale(), "mail_pause_after_creation.text",
				new Object[] { paramHandler.getInstanceName(),
						paramHandler.getServiceSetting(VMPropertyHandler.REQUESTING_USER), details,
						eventLink.toString() });
