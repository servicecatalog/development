...
public class SnapshotActions extends Actions {
	...
	private static final Logger logger = LoggerFactory.getLogger(SnapshotActions.class);
	...
	@StateMachineAction
	public String deleteSnapshots(String instanceId, ProvisioningSettings settings,
			@SuppressWarnings("unused") InstanceStatus result) {

		VMPropertyHandler ph = new VMPropertyHandler(settings);
		try (VMwareClient client = new VMwareClientFactory("en").getInstance(ph);) {

			ManagedObjectReference snapshot = client.findSnapshot(ph.getInstanceName(),
					ph.getServiceSetting(VMPropertyHandler.SNAPSHOT_ID));
			if (snapshot == null) {
				logger.info("Found no old snapshot to delete for instance " + instanceId + ", virtual system '"
			...
			ManagedObjectReference removeTask = client.getService().removeSnapshotTask(snapshot, removeChildren,
					consolidate);

			ph.setTask(client.retrieveTaskInfoKey(removeTask));
			return EVENT_RUN;
		} catch (Exception e) {
			String message = "Failed to delete old snapshot for instance " + instanceId + " and snapshot id "
					+ ph.getServiceSetting(VMPropertyHandler.SNAPSHOT_ID);
	...
	@StateMachineAction
	public String createSnapshot(String instanceId, ProvisioningSettings settings,
			@SuppressWarnings("unused") InstanceStatus result) {
		...
		try {
			...
			ManagedObjectReference vm = client.getVirtualMachine(ph.getInstanceName());
			String description = "Instance ID: " + instanceId + ", Snapshot date: " + (new Date());
			boolean dumpMemory = false;
			boolean quiesceFileSystem = true;
			ManagedObjectReference task = client.getService().createSnapshotTask(vm, SNAPSHOT_NAME, description,
					dumpMemory, quiesceFileSystem);

			ph.setTask(client.retrieveTaskInfoKey(task));
			return EVENT_RUN;
		} catch (Exception e) {
			String message = "Failed to create snapshot for instance " + instanceId;
			logger.error(message, e);
			ph.setSetting(VMPropertyHandler.SM_ERROR_MESSAGE, message);
			return EVENT_FAILED;
		} finally {
	...
	protected String successfulTask(TaskInfo taskInfo, VMPropertyHandler ph) {
		...
		if (TASK_NAME_CREATE_SNAPSHOT.equals(taskInfo.getName())) {
			ManagedObjectReference mor = (ManagedObjectReference) taskInfo.getResult();
	...
	public String finish(String instanceId, ProvisioningSettings settings, InstanceStatus result) {
		...
		VMPropertyHandler ph = new VMPropertyHandler(settings);
		logger.info("Created snapshot for instance " + instanceId + " with snapshot id "
				+ ph.getServiceSetting(VMPropertyHandler.SNAPSHOT_ID));
		return EVENT_SUCCESS;
	}
