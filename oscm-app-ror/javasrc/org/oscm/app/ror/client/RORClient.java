...
public class RORClient {
    ...
    public List<NameValuePair> createParameters(
        ...
        params.add(new BasicNameValuePair(LParameter.VERSION, "2.0"));
        params.add(new BasicNameValuePair(LParameter.LOCALE, locale));
    ...
    public XMLConfiguration execute(HashMap<String, String> values)
        ...
        try {
            ...
            List<NameValuePair> params = createParameters(values);
            URI uri = new URIBuilder(get.getURI()).addParameters(params)
                    .build();
            ...
            getRespStream(result, httpResponse);
        ...
        } catch (RuntimeException | IOException | ConfigurationException
                | URISyntaxException e) {
            throw new RORException(
                    "Command failed: " + values.get(LParameter.ACTION)
        ...
        if (!useProxyByPass(this.apiUrl)) {
            if (proxyHost != null && proxyPortInt > 0) {
                ...
                if (proxyUser != null && proxyUser.length() > 0
                    ...
                    clientBuilder.setProxyAuthenticationStrategy(
    ...
    private void getRespStream(XMLConfiguration result, HttpResponse response)
    ...
    public List<LPlatformDescriptor> listLPlatformDescriptors()
        ...
        if (result != null) {
            List<HierarchicalConfiguration> descriptors = result
                    .configurationsAt(
                            "lplatformdescriptors.lplatformdescriptor");
