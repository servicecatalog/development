...
## Contributions
All contributions are welcome - Open Service Catalog Manager uses the Apache 2.0 license and requires the contributor to agree with the OSCM Individual CLA (ICLA, see above ICLA.txt). If the contributor submits patches on behalf of a company, then additionally the OSCM Corporate CLA (CCLA, see above CCLA.txt) must be agreed. Even if the contributor is included in such CCLA, she/he is still required to agree with the ICLA. To submit the CLAs please:
