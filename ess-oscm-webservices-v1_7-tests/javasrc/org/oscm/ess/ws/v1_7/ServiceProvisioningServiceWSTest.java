...
import com.fujitsu.bss.types.enumtypes.PricingPeriod;
...
import com.fujitsu.bss.types.exceptions.UpdateConstraintException;
...
public class ServiceProvisioningServiceWSTest {
    ...
    @Test
    public void exportTechnicalServiceParameterDefiniton_CheckNoBillingAdapter()
            throws Exception {

        VOTechnicalService technicalService = setup.createTechnicalService();
        List<VOTechnicalService> technicalServices = new ArrayList<VOTechnicalService>();
        technicalServices.add(technicalService);
        byte[] content = setup.exportTechnicalService(technicalServices);
        String xmlString = new String(content);
        assertFalse(
                "Exported xml must not contain billing adapter information",
                xmlString.contains("billingIdentifier"));
    }
