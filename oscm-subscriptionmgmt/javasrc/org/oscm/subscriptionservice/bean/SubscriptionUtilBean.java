...
public class SubscriptionUtilBean {
	...
	private static final Log4jLogger LOG = LoggerFactory.getLogger(ManageSubscriptionBean.class);
	...
	protected List<PlatformUser> getCustomerAdminsAndSubscriptionOwner(Subscription subscription) {
		List<PlatformUser> users = getOrganizationDao().getOrganizationAdmins(subscription.getOrganizationKey());
	 ...
	 */
	List<PlatformUser> getCustomerAndTechnicalProductAdminForSubscription(Subscription subscription) {

		List<PlatformUser> users = getCustomerAdminsAndSubscriptionOwner(subscription);
		for (PlatformUser orgAdmin : subscription.getProduct().getTechnicalProduct().getOrganization()
	...
	List<Uda> getExistingUdas(Subscription subscription) {
		List<Uda> existingUdas = getUdaAccess().getExistingUdas(subscription.getOrganization().getKey(),
				subscription.getKey(), subscription.getProduct().getSupplierOrResellerTemplate().getVendor());
		return existingUdas;
	}

	void setSubscriptionOwner(Subscription subscriptionToModify, String ownerId,
			boolean validateOrganization) throws ObjectNotFoundException, OperationNotPermittedException {
		if (ownerId != null && ownerId.length() != 0) {
			// PlatformUser owner = idManager.getPlatformUser(ownerId,
			// dataManager
			// .getCurrentUser().getTenantId(), validateOrganization);
			//if the method is called from platform operator (APP), the calling user tenant is different from that of the owner
			Tenant tenant = subscriptionToModify.getOrganization().getTenant();
			String tenantId = null;
			if (tenant != null) {
				tenantId = tenant.getTenantId();
			}
			PlatformUser owner = idManager.getPlatformUser(ownerId, tenantId, validateOrganization);
	...
	void setSubscriptionUnit(Subscription subscriptionToModify, String unitKey) throws ObjectNotFoundException {
		if (unitKey != null && unitKey.trim().length() != 0) {
			UserGroup unit = dataManager.getReference(UserGroup.class, Long.valueOf(unitKey).longValue());
	 ...
	 *             based
	 */
	void validateTechnoloyProvider(Subscription subscription) throws OperationNotPermittedException {
		Organization tp = dataManager.getCurrentUser().getOrganization();
		if (!tp.getTechnicalProducts().contains(subscription.getProduct().getTechnicalProduct())) {
	 ...
	 */
	Subscription findSubscription(String subscriptionId, String organizationId) throws ObjectNotFoundException {
		Organization organization = new Organization();
		organization.setOrganizationId(organizationId);
		organization = (Organization) dataManager.getReferenceByBusinessKey(organization);

		Subscription subscription = new Subscription();
		subscription.setSubscriptionId(subscriptionId);
		subscription.setOrganization(organization);
		subscription = (Subscription) dataManager.getReferenceByBusinessKey(subscription);
		return subscription;
	}
	...
	protected void removeUsageLicenses(Subscription subscription) {
		...
		boolean needToSendMails = !(subscription.getStatus() != SubscriptionStatus.ACTIVE
				|| subscription.getProduct().getTechnicalProduct().getAccessType() == ServiceAccessType.DIRECT);
		List<UsageLicense> usageLicenses = new ArrayList<UsageLicense>(subscription.getUsageLicenses());
	...
	private void sendUserRemoveMailMessages(Subscription subscription, List<UsageLicense> usageLicenses) {
		...
		for (UsageLicense license : usageLicenses) {
			payload.addMailObjectForUser(license.getUser().getKey(), EmailType.SUBSCRIPTION_USER_REMOVED,
	...
	protected void sendTechnicalServiceErrorMail(Subscription subscription) {
		...
		List<PlatformUser> admins = tp.getOrganization().getOrganizationAdmins();
		for (PlatformUser admin : admins) {
			try {
				commService.sendMail(admin, EmailType.SUBSCRIPTION_TERMINATE_TECHNICAL_SERVICE_ERROR,
						new Object[] { tp.getTechnicalProductId(), subscription.getProductInstanceId() },
						subscription.getMarketplace());
			} catch (MailOperationException e) {
				// only log the exception and proceed
				LOG.logWarn(Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.WARN_SENDING_ERROR_MAIL_FAILED);
			}
		}
	}

	void deleteModifiedEntityForSubscription(Subscription subscription) {
		getModifiedEntityDao().deleteModifiedEntityForSubscription(subscription);
		dataManager.flush();
	}
