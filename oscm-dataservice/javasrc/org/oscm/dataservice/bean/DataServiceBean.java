...
public class DataServiceBean implements DataService {
	...
	public DomainObject<?> find(DomainObject<?> idobj) {
		setThreadLocals();
		String[] businessKeyElements = PersistenceReflection.getBusinessKey(DomainObject.getDomainClass(idobj));
		...
		try {
			...
			qrykey.append(")");
			String msgText = "Non-Unique Business Key Search for " + PersistenceReflection.getDomainClassName(idobj)
	...
	public PlatformUser find(PlatformUser pu) {
		...
		try {
			...
			String qrykey = "(" + "tenantId='" + pu.getTenantId() + "'," + "userId='" + pu.getUserId() + ")";
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.MANDATORY)
	public DomainObject<?> getReferenceByBusinessKey(DomainObject<?> findTemplate) throws ObjectNotFoundException {
		setThreadLocals();
		DomainObject<?> result;
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.MANDATORY)
	public <T extends DomainObject<?>> T getReference(Class<T> objclass, long id) throws ObjectNotFoundException {
		setThreadLocals();
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.MANDATORY)
	public void validateBusinessKeyUniqueness(DomainObject<?> obj) throws NonUniqueBusinessKeyException {
		...
		if (businessKeyMap != null) {
			...
			if (search != null && obj.getKey() != search.getKey()) {
				...
				logger.logWarn(Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.WARN_NON_UNIQUE_BUSINESS_KEY_WITH_TYPE,
	...
	private PlatformUser getUserFromSessionContext(boolean lookupOnly) {
		...
		try {
			...
			InvalidUserSession ius = new InvalidUserSession(
					"User with key '" + name + "' cannot be found! The user might have been deleted by administrator.",
					e);
	 ...
	 */
	private PlatformUser loadUserFromSessionContext(boolean lookupOnly) throws ObjectNotFoundException {
	...
	private boolean mayBeWebServiceSSLContext() {
		...
		try {
			...
			return principal != null && principal.getName() != null && principal.getName().indexOf('=') > 0;
		} catch (Exception e) {
			return false;
		}
	}
	...
	private Organization getOrganizationForDistinguishedName(boolean lookupOnly) {
		final Set<String> principalNames = Collections.singleton(webServiceContext.getUserPrincipal().getName());
		final TypedQuery<Organization> query = createNamedQuery("Organization.getOrgsForDN", Organization.class);
	...
	private boolean checkOrgDeregistration(Organization org, boolean lookupOnly) {
		if (org.getDeregistrationDate() != null) {
			...
			InvalidUserSession ius = new InvalidUserSession(
					"Organization with key '" + org.getKey() + "' is deregistered!");
			logger.logError(Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG, ius,
					LogMessageIdentifier.ERROR_USER_OPERATE_NOT_PERMITTED, org.getOrganizationId());
			throw ius;
		}
		return true;
	}
	 ...
	 *         classEnum
	 */
	private String getBusinessKey(Map<String, String> businessKeyMap, DomainObjectException.ClassEnum classEnum) {
	...
	DataSet convertToDataSet(ResultSet rs, int max) throws SQLException {
		...
		for (int index = 1; index <= rs.getMetaData().getColumnCount(); index++) {
			dataSet.getMetaData().add(index, rs.getMetaData().getColumnName(index),
					rs.getMetaData().getColumnTypeName(index), rs.getMetaData().getColumnType(index));
		}
		int i = 0;
		while (i++ < max && rs.next()) {
	...
	private void closeResultSet(ResultSet rs) {
		if (rs != null) {
			try {
				...
				logger.logError(Log4jLogger.SYSTEM_LOG, ignore, LogMessageIdentifier.ERROR_CLOSE_RESOURCE_FAILED);
			}
		}
	}
